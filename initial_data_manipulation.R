...
survey_data <- bind_rows(collated_fish %>% 
            ...
            filter(date.dif <= 14) %>% 
            select(c(siteID, min.date)), #site ID and date for surveys where all transects conducted within 14 days
          collated_fish %>% 
            group_by(siteID) %>% 
            summarise(min.date = min(dateN), max.date = max(dateN)) %>% 
            mutate(date.dif = max.date - min.date) %>%
            filter(date.dif > 14) %>%
            select(c(siteID, min.date)), #site ID and first date for surveys where transects conducted over >14 days
          collated_fish %>% 
            group_by(siteID) %>% 
            summarise(min.date = min(dateN), max.date = max(dateN)) %>% 
            mutate(date.dif = max.date - min.date) %>%
            filter(date.dif > 14) %>%
            mutate(min.date = max.date - 14) %>%
            select(c(siteID, min.date))
) #site ID and last date -14 days (to give earliest possible date) for surveys where transects conducted over >14 days
...
for (i in 1:length(survey_data$siteID)){
  survey_data$dateN[i] <- 0.5*(max(collated_fish$dateN[collated_fish$siteID == survey_data$siteID[i] &
                                                       collated_fish$dateN <= survey_data$min.date[i] + 14]) + 
...
###CHECKING DIFFERENCES IN SURVEY DATES###
#all transects taken within 14 days of each other considered the same survey
#get coral and benthic transect dates
for (i in 1:length(survey_data$siteID)){
  survey_data$ben.date1[i] <- min(transect_benthic$dateN[transect_benthic$siteID == survey_data$siteID[i] 
                                                                & transect_benthic$dateN <= survey_data$dateN[i] + 14
                                                                & transect_benthic$dateN >= survey_data$dateN[i] - 14], na.rm = T)
}

for (i in 1:length(survey_data$siteID)){
  survey_data$ben.date2[i] <- max(transect_benthic$dateN[transect_benthic$siteID == survey_data$siteID[i] 
                                                         & transect_benthic$dateN <= survey_data$dateN[i] + 14
                                                         & transect_benthic$dateN >= survey_data$dateN[i] - 14], na.rm = T)
}

for (i in 1:length(survey_data$siteID)){
  survey_data$coral.date1[i] <- min(collated_coral$dateN[collated_coral$siteID == survey_data$siteID[i] 
                                                         & collated_coral$dateN <= survey_data$dateN[i] + 14
                                                         & collated_coral$dateN >= survey_data$dateN[i] - 14], na.rm = T)
}

for (i in 1:length(survey_data$siteID)){
  survey_data$coral.date2[i] <- max(collated_coral$dateN[collated_coral$siteID == survey_data$siteID[i] 
                                                         & collated_coral$dateN <= survey_data$dateN[i] + 14
                                                         & collated_coral$dateN >= survey_data$dateN[i] - 14], na.rm = T)
}

#calculate date differences either side of fish transects
survey_data$ben.date.dif1<-abs(survey_data$first.date-survey_data$ben.date1) #time between first fish transect and first ben transect
survey_data$ben.date.dif2<-abs(survey_data$last.date-survey_data$ben.date1) #last fish transect and first ben transect
survey_data$ben.date.dif3<-abs(survey_data$first.date-survey_data$ben.date2) #first fish transect and last ben transect
survey_data$ben.date.dif4<-abs(survey_data$last.date-survey_data$ben.date2) #last fish transect and last ben transect

survey_data$coral.date.dif1<-abs(survey_data$first.date-survey_data$coral.date1) #first fish transect and first coral transect
survey_data$coral.date.dif2<-abs(survey_data$last.date-survey_data$coral.date1) #last fish transect and first coral transect
survey_data$coral.date.dif3<-abs(survey_data$first.date-survey_data$coral.date2) #first fish transect and last coral transect
survey_data$coral.date.dif4<-abs(survey_data$last.date-survey_data$coral.date2) #last fish transect and last coral transect

#look for any surveys where coral/benthic transects conducted >14 days before/after fish transects
length(survey_data$siteID[survey_data$ben.date.dif1>14 & !is.na(survey_data$ben.date.dif1)])
length(survey_data$siteID[survey_data$ben.date.dif2>14 & !is.na(survey_data$ben.date.dif2)])
length(survey_data$siteID[survey_data$ben.date.dif3>14 & !is.na(survey_data$ben.date.dif3)])
length(survey_data$siteID[survey_data$ben.date.dif4>14 & !is.na(survey_data$ben.date.dif4)])

length(survey_data$siteID[survey_data$coral.date.dif1>14 & !is.na(survey_data$coral.date.dif1)])
length(survey_data$siteID[survey_data$coral.date.dif2>14 & !is.na(survey_data$coral.date.dif2)])
length(survey_data$siteID[survey_data$coral.date.dif3>14 & !is.na(survey_data$coral.date.dif3)])
length(survey_data$siteID[survey_data$coral.date.dif4>14 & !is.na(survey_data$coral.date.dif4)])
